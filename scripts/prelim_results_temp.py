#!/usr/bin/env python3
"""Quick-and-dirty preliminary comparison for SVM + SetFit + Zero-shot outputs."""
...
def summarize():
    ...
    lines.append("- Note: zero-shot is a single run at 0% train fraction (no fitting).\n")
    ...
    for frac in common_fracs:
        ...
        lines.append(
            f"{frac_key(frac)} | {f(sf_m)} | {f(sv_m)} | {f(zero_macro_mean)} | {f(sf_m - sv_m)} | {f(sf_m - zero_macro_mean)} | {f(sv_m - zero_macro_mean)}"
        )
    lines.append("")
    ...
    paragraph = (
        "Preliminary results across SVM, SetFit, and zero-shot indicate a clear ordering for current runs: "
        "SetFit performs best, SVM is second, and zero-shot is substantially lower on macro-F1. Using all available seeds, "
        ...
        "which is far below both trained approaches. Seed-42 comparisons across common fractions show the same trend, and "
        f"at {frac_key(best_frac)} the seed-42 gap is SetFit {f(sf_best)} vs SVM {f(sv_best_seed42)} vs zero-shot {f(zero_macro_mean)}. "
    ...
    output_path = OUTPUT_DIR / "prelim_svm_setfit_summary.txt"
